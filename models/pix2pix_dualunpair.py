...
class Pix2PixModel(torch.nn.Module):
    ...
    def forward(self, data, mode):
        input_semantics, real_image, fine_label, input_semantics_fine, real_image_fine = self.preprocess_input(data)

        if mode == 'generator':
            g_loss, generated = self.compute_generator_loss(
                input_semantics, real_image, fine_label, input_semantics_fine, real_image_fine)
            return g_loss, generated
        elif mode == 'discriminator':
            d_loss = self.compute_discriminator_loss(
                input_semantics, real_image, input_semantics_fine, real_image_fine)
            return d_loss
            ...
            with torch.no_grad():
                fake_image, _, seg_out, fake_fine_image= self.generate_fake(input_semantics, real_image, input_semantics_fine)
                seg_out = torch.argmax(seg_out,dim=1).unsqueeze(0)
                # seg_out = seg_out.float()
                # seg_out = (seg_out/255.0 * 2.0) - 1
                # print(seg_out.size())
                seg_label = self.FloatTensor(1, 35, 256, 512).zero_()
                seg_label = seg_label.scatter_(1, seg_out, 1.0)
    ...
    def initialize_networks(self, opt):
        ...
        netD_fine = networks.define_D(opt) if opt.isTrain else None
    ...
    def preprocess_input(self, data):
        ...
        if self.opt.box_unpair:
            ...
            data['image_fine'] = data['image_fine'].cuda()
        ...
        return input_semantics, data['image'], data['fine_label'], input_semantics_fine,data['image_fine']

    def compute_generator_loss(self, input_semantics, real_image, fine_label,input_semantics_fine, real_image_fine):
        G_losses = {}

        fake_image, KLD_loss, seg_out, fake_fine_image = self.generate_fake(
        ...
        pred_fake_fine, pred_real_fine = self.discriminate_fine(
            input_semantics_fine, fake_fine_image, real_image_fine)
    ...
    def compute_discriminator_loss(self, input_semantics, real_image, input_semantics_fine, real_image_fine):
        D_losses = {}
        with torch.no_grad():
            fake_image, _, _, fake_fine_image = self.generate_fake(input_semantics, real_image,input_semantics_fine)
